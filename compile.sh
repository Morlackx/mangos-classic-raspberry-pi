...
sudo apt-get --allow-unauthenticated install -y build-essential git m4 sudo python clang-5.0 lldb-5.0 lld-5.0 make cmake libboost1.55-dev libboost-system1.55-dev libboost-program-options1.55-dev libboost-thread1.55-dev libboost-regex1.55-dev
if [ -z ${ISTRAVIS+x} ]; 
then
